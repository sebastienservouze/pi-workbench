...
Par défaut, l’en-tête de l’outil expose ses arguments formatés et les tailles de l’appel et de la sortie au survol ; ces informations sont également disponibles au lecteur d’écran. Sa sortie est affichée directement, dans une zone défilable de quatre lignes au plus.
...
3. Retournez un `ToolCallPresentation` :
   - `headerDetail` affiche un détail compact dans l’en-tête ; fournissez le texte complet dans `title` pour le lecteur d’écran ;
